...
export default function SectorIntensity({ filters }) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get("https://analytical-dashboard-backend.netlify.app/api/records/sector/intensity", { params: filters })
      .then((res) => {
        setData(res.data);
        setLoading(false);
      })
      .catch(() => {
        setError("Failed to load sector intensity");
        setLoading(false);
      });
  }, [filters]);
